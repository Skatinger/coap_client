...
extern int z_impl_gpio_enable_callback(struct device * port, gpio_pin_t pin);
static inline int gpio_enable_callback(struct device * port, gpio_pin_t pin)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
...
extern int z_impl_gpio_disable_callback(struct device * port, gpio_pin_t pin);
static inline int gpio_disable_callback(struct device * port, gpio_pin_t pin)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (int) arch_syscall_invoke2(*(uintptr_t *)&port, *(uintptr_t *)&pin, K_SYSCALL_GPIO_DISABLE_CALLBACK);
	}
#endif
	compiler_barrier();
	return z_impl_gpio_disable_callback(port, pin);
}
