...
extern u8_t z_impl_counter_get_num_of_channels(const struct device * dev);
static inline u8_t counter_get_num_of_channels(const struct device * dev)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (u8_t) arch_syscall_invoke1(*(uintptr_t *)&dev, K_SYSCALL_COUNTER_GET_NUM_OF_CHANNELS);
	}
#endif
	compiler_barrier();
...
extern u32_t z_impl_counter_get_frequency(const struct device * dev);
static inline u32_t counter_get_frequency(const struct device * dev)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (u32_t) arch_syscall_invoke1(*(uintptr_t *)&dev, K_SYSCALL_COUNTER_GET_FREQUENCY);
	}
#endif
	compiler_barrier();
...
extern u32_t z_impl_counter_us_to_ticks(const struct device * dev, u64_t us);
static inline u32_t counter_us_to_ticks(const struct device * dev, u64_t us)
{
	...
	if (z_syscall_trap()) {
		...
		parm0.val = us;
		return (u32_t) arch_syscall_invoke3(*(uintptr_t *)&dev, parm0.split.lo, parm0.split.hi, K_SYSCALL_COUNTER_US_TO_TICKS);
	}
#endif
	compiler_barrier();
...
extern u64_t z_impl_counter_ticks_to_us(const struct device * dev, u32_t ticks);
...
extern u32_t z_impl_counter_get_max_top_value(const struct device * dev);
static inline u32_t counter_get_max_top_value(const struct device * dev)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		return (u32_t) arch_syscall_invoke1(*(uintptr_t *)&dev, K_SYSCALL_COUNTER_GET_MAX_TOP_VALUE);
	}
#endif
	compiler_barrier();
...
extern int z_impl_counter_get_value(struct device * dev, u32_t * ticks);
static inline int counter_get_value(struct device * dev, u32_t * ticks)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
...
extern int z_impl_counter_set_channel_alarm(struct device * dev, u8_t chan_id, const struct counter_alarm_cfg * alarm_cfg);
static inline int counter_set_channel_alarm(struct device * dev, u8_t chan_id, const struct counter_alarm_cfg * alarm_cfg)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
...
extern int z_impl_counter_cancel_channel_alarm(struct device * dev, u8_t chan_id);
static inline int counter_cancel_channel_alarm(struct device * dev, u8_t chan_id)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
